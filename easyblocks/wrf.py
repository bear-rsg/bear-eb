...
class EB_WRF(EasyBlock):
    ...
    def extra_options():
        extra_vars = {
            ...
            'withchem': [False, "Build with Chem support", CUSTOM],
        }
    ...
    def configure_step(self):
        ...
        # Gavin addition to enable WRF with chem
        if self.cfg['withchem']:
            ...
            env.setvar('NETCDF_LIB', os.path.join(netcdff, "lib"))
            # Flex lib additions
            flex = get_software_root('flex')
            env.setvar('FLEX', os.path.join(flex, "bin"))
            env.setvar('FLEX_LIB_DIR', os.path.join(flex, "lib"))
            # YACC addition from Bison
            yacc = get_software_root('Bison')
            env.setvar('YACC', os.path.join(yacc, "bin/yacc -d"))
